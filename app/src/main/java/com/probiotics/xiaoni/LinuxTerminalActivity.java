...
public final class LinuxTerminalActivity extends Activity {
    ...
    @Override protected void onActivityResult(int requestCode, int resultCode, Intent data) { super.onActivityResult(requestCode, resultCode, data); if (requestCode == PICK_ROOTFS && resultCode == RESULT_OK && data != null && data.getData() != null) importRootfs(data.getData()); }
     private void importRootfs(Uri uri) { status.setText("正在导入本地 rootfs..."); worker.execute(() -> { try { String name = String.valueOf(uri); String suffix = name.contains("xz") ? ".tar.xz" : ".tar.gz"; File archive = new File(LinuxImages.root(this), "local-rootfs" + suffix); LinuxImages.root(this).mkdirs(); try (InputStream in = getContentResolver().openInputStream(uri); OutputStream out = new FileOutputStream(archive)) { if (in == null) throw new IOException("无法读取文件"); byte[] buffer = new byte[65536]; int n; while ((n = in.read(buffer)) != -1) out.write(buffer, 0, n); } File destination = LinuxImages.environment(this, LinuxImages.LOCAL); extract(archive, destination); snapshotPackages(destination); runOnUiThread(() -> { status.setText("本地 rootfs 已安装"); refreshList(); }); } catch (Exception e) { runOnUiThread(() -> status.setText("本地安装失败: " + e.getMessage())); } }); }

      private void download(LinuxImages.Image image, LinearLayout row) { ProgressBar progress = (ProgressBar) row.getChildAt(3); TextView progressText = (TextView) row.getChildAt(4); Button action = (Button) ((LinearLayout) row.getChildAt(0)).getChildAt(1); progress.setVisibility(View.VISIBLE); progressText.setVisibility(View.VISIBLE); progressText.setText("正在连接服务器..."); action.setEnabled(false); action.setText("下载中"); worker.execute(() -> { File environment = LinuxImages.environment(this, image); try { LinuxImages.root(this).mkdirs(); java.net.HttpURLConnection c = (java.net.HttpURLConnection) new java.net.URL(image.url).openConnection(); c.setConnectTimeout(15000); c.setReadTimeout(30000); c.connect(); if (c.getResponseCode() / 100 != 2) throw new IOException("HTTP " + c.getResponseCode()); long total = c.getContentLengthLong(); File archive = LinuxImages.archive(this, image); try (InputStream in = c.getInputStream(); FileOutputStream out = new FileOutputStream(archive)) { byte[] b = new byte[262144]; long done = 0; int n; while ((n = in.read(b)) != -1) { out.write(b, 0, n); done += n; final long downloaded = done; int value = total > 0 ? (int) (done * 100 / total) : 0; runOnUiThread(() -> { if (total > 0) progress.setProgress(value); progressText.setText(total > 0 ? "已下载 " + formatBytes(downloaded) + " / " + formatBytes(total) + "（" + value + "%）" : "已下载 " + formatBytes(downloaded)); }); } } runOnUiThread(() -> progressText.setText("下载完成，正在解压...")); deleteTree(environment); extract(archive, environment); if (!LinuxImages.hasUsableShell(environment)) throw new IOException("rootfs 中没有可用的 shell"); snapshotPackages(environment); runOnUiThread(this::refreshList); } catch (Exception e) { deleteTree(environment); runOnUiThread(() -> { progressText.setText("下载失败: " + e.getMessage()); action.setEnabled(true); action.setText("重试"); }); } }); }
    ...
    private void startShell(LinuxImages.Image image) { terminalVisible = true; status.setText("正在准备 ROOT Linux 终端..."); worker.execute(() -> { try { File environment = LinuxImages.environment(this, image); File archive = LinuxImages.archive(this, image); if (!environment.isDirectory() && archive.isFile()) { runOnUiThread(() -> status.setText("正在从下载包恢复 " + image.name + "...")); extract(archive, environment); } runRootShellStrict(image); } catch (Exception e) { runOnUiThread(() -> status.setText("启动失败: " + e.getMessage())); } }); }
    ...
    private final class PtySessionClient implements TerminalSessionClient {
        ...
        PtySessionClient(TextView state) { this.state = state; }
         public void onTextChanged(TerminalSession s) {
             ...
             if (view != null) runOnUiThread(view::invalidate);
         }
        public void onTitleChanged(TerminalSession s) { }
        public void onSessionFinished(TerminalSession s) { runOnUiThread(() -> state.setText("已退出")); }
    ...
    @Override protected void onDestroy() { setTerminalWakeLock(false); stopShell(); worker.shutdownNow(); sizeWorker.shutdownNow(); super.onDestroy(); }
}
